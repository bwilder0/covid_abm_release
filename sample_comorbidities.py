...
def sample_joint(age, p_diabetes, p_hyp):
    ...
    p_hyp_given_diabetes = 0.5*(p_hyp[age] < 0.5) + p_hyp[age]*(p_hyp[age] > 0.5)
    p_hyp_given_not_diabetes = (p_hyp[age] - p_hyp_given_diabetes*p_diabetes[age])/(1 - p_diabetes[age])
    diabetes_status = (np.random.rand(age.shape[0]) < p_diabetes[age]).astype(np.int)
    hyp_status = np.zeros(age.shape[0], dtype=np.int)
    hyp_status[diabetes_status == 1] = np.random.rand((diabetes_status == 1).sum()) < p_hyp_given_diabetes[diabetes_status == 1]
    hyp_status[diabetes_status == 0] = np.random.rand((diabetes_status == 0).sum()) < p_hyp_given_not_diabetes[diabetes_status == 0]
...
def p_comorbidity(country, comorbidity, warning=False):
    ...
    if country=='NYC':
        ...
        #https://www.cdc.gov/diabetes/research/modeling/states/newyork.html
        if comorbidity=='diabetes':
            for i in range(101):
                if i <= 17:
                    prevalence[i] = 0.0025 # figure from overall US
                elif i <= 49:
                    prevalence[i] = 0.067
                elif i <= 64:
                    prevalence[i] = 0.247
        ...
        # https://a860-gpp.nyc.gov/bitstream/gpp/8713/1/EpiDataBrief_82_Hypertension_Prevalence_2016.pdf
        elif comorbidity=='hypertension':
            for i in range(101):
                if i <= 17:
                    prevalence[i] = 0.04 # figure from overall US
                elif i <= 24:
                    prevalence[i] = 0.07
                elif i <= 44:
                    prevalence[i] = 0.12
